...
function landingFor(id) {
  ...
  if (id === 'cover') target = 0;
  ...
  else if (id === 'context' && scrollyContainer && scrollyGraphic) {
    // The moment the text and the diagram pin, with the scene fully up:
    // the first step, whole, and where it stays for its share of the run.
    const stickyTop = parseFloat(getComputedStyle(scrollyGraphic).top) || 0;
    const pin = scrollyContainer.getBoundingClientRect().top + window.scrollY - stickyTop;
    target = Math.max(pin, y - vh * FADE_TO);
  }
  else if (id === 'epilogue') target = y;   // it is the last screen, whole
  else if (el.querySelector(':scope > .page-width > h2')) {
    ...
    const h2 = el.querySelector(':scope > .page-width > h2');
    target = h2.getBoundingClientRect().top + window.scrollY - NAV_H() - HEADING_GAP;
  }
  else target = y - NAV_H();                // the section's top under the nav
...
['wheel', 'touchstart', 'keydown'].forEach(type =>
  window.addEventListener(type, () => { if (jumping && !cut.classList.contains('is-on')) jumping = null; },
    { passive: true }));
